...
class EmlaLockApi:
    ...
    async def request(self, endpoint: str, **params: Any) -> dict[str, Any]:
        ...
        if self.holder_api_key:
            # EmlaLock requires this exact request parameter for holder actions.
        ...
        try:
            async with self.session.get(url, timeout=20) as response:
                ...
                # EmlaLock normally uses HTTP 4xx for errors, but also guard
                # against an error object returned with a successful status.
